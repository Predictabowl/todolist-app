...
class JPAEntityConstraintsTest {
	...
	void test_deleteUser_cascadeOnProjects() {
		...
		List<ProjectJPA> resultList = entityManager.getEntityManager().createQuery("from ProjectJPA",ProjectJPA.class).getResultList();
	...
	void test_deleteProject_cascadeOnTasks() {
		...
		List<TaskJPA> resultList = entityManager.getEntityManager().createQuery("from TaskJPA",TaskJPA.class).getResultList();
		assertThat(resultList).isEmpty();
	}
